...
The project uses Nix flakes for reproducible development. To get started:

```bash
# Enter the development shell (includes all benchmark tools + dev tools)
nix develop
...
- All benchmark tools (p7zip, fio, stress-ng, etc.)
- Python 3 with the standard library
- Development tools: prek (pre-commit), ruff, mypy, typos, nixpkgs-fmt
...
- **typos**: Spell checking across all text files
- **nixpkgs-fmt**: Nix code formatting
- **trailing-whitespace**, **end-of-file-fixer**: General text cleanup
...
The pre-commit configuration is tailored for this Python + Nix project and will help catch issues before they're committed.

### Code style

- Python code follows PEP 8 with ruff formatting (88 character line length)
